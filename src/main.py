    ...
    try:
        ...
        if player_card is not None:
            ...
            await inter.edit_original_message(
                "Could not find character: " + str(character_name)
            )
...
async def outfit(
    ...
    try:
        ...
        if outfit_card is not None:
            ...
            await inter.edit_original_message(
                "Could not find outfit: " + str(name) + str(tag)
            )
    except Exception as e:
        await inter.edit_original_message("Hmm, looks like something went wrong.")
        logging.exception(e)
...
async def vote(
    ...
    discord_emojis = list(
        set(re.compile(r"<:.*:[0-9]*>").findall(message.content))
    )  # some magic to delete duplicates
