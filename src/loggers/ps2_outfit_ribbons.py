...
class Ps2OutfitRibbonLogger(commands.Cog):
    def __init__(
        ...
        self.logged_ribbons = [
            Ps2RibbonIDs.PlatoonLeadership,
            Ps2RibbonIDs.SquadLeadership,
        ]
        ...
        try:
            ...
            logging.error(
                "Failed to initialize PlanetSide outfit ribbon logger",
                exc_info=exception,
            )
    ...
    async def ps2_outft_ribbon_saver(self) -> None:
        for outfit_id in self.monitored_outfits:
            ...
            ribbons = await Census.get_outfit_member_ribbons(
                outfit, self.logged_ribbons
            )
                    ...
                    for achievement in ribbons[ribbon]:
                        new_ribbon_ts_entry = PS2RibbonTS(
                            ...
                            meta=Ps2RibbonMetaData(
                                ribbon_id=achievement.achievement_id,
                                character_id=achievement.character_id,
                            ),
                        )
                        last_record = (
                            await PS2RibbonTS.find(
                                {
                                    "meta": Ps2RibbonMetaData(
                                        ribbon_id=achievement.achievement_id,
                                        character_id=achievement.character_id,
                            ...
                            .sort(-PS2RibbonTS.timestamp)
                            .first_or_none()
                        )

                        if (
                            last_record is None
                            or last_record.ribbon_count
                            != new_ribbon_ts_entry.ribbon_count
                        ):
                            await PS2RibbonTS.insert_one(document=new_ribbon_ts_entry)
                    print(
                        f"inserted {len(ribbons[ribbon])} ribbons for ribbon {ribbon}"
                    )
