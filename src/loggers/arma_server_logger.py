...
class ArmALogger(Cog):
    def __init__(self, bot: Bot):
        try:
            self.bot = bot
            self.server_query = SteamQuery(
                os.getenv("ARMA_HOST"), int(os.getenv("ARMA_QUERY_PORT"))
            )
    ...
    async def log_server_status(self):
        ...
        try:
            ...
            last = (
                await OnlineFUArmaPlayers.find()
                .sort(-OnlineFUArmaPlayers.timestamp)
                .first_or_none()
            )
            new = OnlineFUArmaPlayers(
                ...
                online_players=[
                    ...
                    for player in self.server_query.query_player_info()
                ],
                mission=mission,
            ...
            if (
                last is None
                or last.online_count != new.online_count
                or new.online_count > 0
            ):
                await new.insert()
        except Exception as exception:
            logging.error(
