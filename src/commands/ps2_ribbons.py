...
class Ps2RibbonsCog(commands.Cog):
    ...
    async def planetside(self, inter: disnake.ApplicationCommandInteraction):
        pass
    ...
    async def top_leaders(
        ...
        ribbon_type: str = commands.param(
            choices=list(Ps2RibbonIDs.__members__.keys()),
            default=Ps2RibbonIDs.SquadLeadership.name,
        ),
        hours: int = 0,
        days: int = 0,
        months: int = 0,
        ...
        embed = self.render_top_ribbon_message(ribbon_diffs, ribbon_type, time_diff)
        await inter.edit_original_message(embed=embed)

    def render_top_ribbon_message(
        self, ribbon_diffs: list[PS2RibbonDiff], ribbon_name: str, time_diff: timedelta
    ) -> disnake.Embed:
        time_string = ""
        if time_diff.days > 0:
        ...
        for index, ribbon_diff in enumerate(ribbon_diffs):
            ...
            table_data.append(
                [
                    medal,
                    place,
                    ribbon_diff.get("diff") or 0,
                    ribbon_diff.get("latest").get("ribbon_count"),
                    ribbon_diff.get("character").get("name"),
                ]
            )

        table_headers = ["", "Place", "Gained", "Total", "Name"]
        ...
        embed_title: str = (
            f"Top {len(ribbon_diffs)} {ribbon_name} in the last {time_string}"
        )
        embed_description: str = f"```{table_str}```"
